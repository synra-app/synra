  ...
  return {
    async readFile(path: string, encoding: BufferEncoding = 'utf-8'): Promise<ReadFileResult> {
      ...
      if (
        ...
        throw new BridgeError(BRIDGE_ERROR_CODES.unauthorized, 'Path is outside the allowed roots.')
      }
      ...
      try {
        ...
        throw new BridgeError(BRIDGE_ERROR_CODES.notFound, 'File does not exist or cannot be read.')
      }
    }
  }
}
